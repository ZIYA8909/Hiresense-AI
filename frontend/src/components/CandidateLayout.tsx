...
export default function CandidateLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: 'Resume Analyzer', href: '/candidate/resume-analyzer', icon: UploadCloud },
    { name: 'Career Coach', href: '/candidate/career-coach', icon: MessageSquare },
    { name: 'AI Mock Interview', href: '/candidate/ai-interview', icon: Play },
    { name: 'Coding Playground', href: '/candidate/playground', icon: Code2 },
  ];
    ...
    <div className="flex h-screen bg-[#1B263B] overflow-hidden text-gray-100 p-4">
            ...
            return (
              <Link
                ...
                className={`flex items-center transition-all ${
                  ...
                  isActive 
                    ? 'bg-[#415A77] text-white scale-105 shadow-lg shadow-[#415A77]/20' 
                    : 'text-gray-400 hover:text-white hover:bg-[#1B263B]/20'
                }`}
              >
                <Icon className={`h-5 w-5 shrink-0 ${isActive ? 'text-white' : 'text-[#B3AF8F]'}`} />
                {isExpanded && (
          ...
          <div className="flex items-center gap-3 w-full">
            ...
            {isExpanded && (
              <div className="overflow-hidden animate-in fade-in duration-200 text-left">
                <div className="text-xs font-bold text-white truncate">{user?.name || 'Arjun Mehta'}</div>
                <div className="text-[10px] text-gray-450 truncate">{user?.email || 'arjun.mehta@gmail.com'}</div>
              </div>
            )}
          </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden pl-4">
        ...
        <header className="h-16 glass-panel border border-[#415A77] rounded-2xl px-6 flex items-center justify-between shrink-0 mb-4 bg-[#1B263B]">
          <div className="text-xs font-bold text-[#B3AF8F] uppercase tracking-widest">
            {navItems.find((item) => item.href === pathname)?.name || 'Portal'}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Compass className="h-3.5 w-3.5 text-[#415A77]" />
              Landing Page
            </Link>
            
            <Link
              href="/candidate/dashboard"
              className="px-3.5 py-1.5 rounded-lg bg-[#415A77] hover:bg-[#a15666] text-white font-bold text-[11px] transition-all flex items-center gap-1.5 shadow-md shadow-[#415A77]/10"
            >
            ...
            <button className="relative p-2 rounded-lg bg-[#1B263B] border border-[#415A77] hover:bg-[#1B263B] transition-all text-gray-400 hover:text-white">
              <Bell className="h-4 w-4 text-[#B3AF8F]" />
              <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-[#B3AF8F]" />
            </button>
            <div className="px-2.5 py-1 rounded bg-[#B3AF8F]/10 border border-[#B3AF8F]/20 text-[#B3AF8F] text-[10px] font-bold uppercase tracking-wide">
              Candidate
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-4 bg-[#1B263B] relative rounded-2xl border border-[#415A77]">
          <div className="absolute top-0 right-0 w-80 h-80 bg-[#415A77]/5 blur-[100px] pointer-events-none rounded-full" />
          {children}
        </main>
      </div>
    </div>
