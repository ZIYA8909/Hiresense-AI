...
public class AuthService {
    ...
    private void sendAuthEvent(String eventType, String email, Map<String, Object> data) {
        try {
            ...
            kafkaTemplate.send("user-events", email, jsonPayload);
        } catch (Exception e) {
            // Log warning but don't fail auth flow (for easy local running without running Kafka)
            System.err.println("[AuthService] Warning: Failed to send Kafka event: " + e.getMessage());
        }
    }
}
